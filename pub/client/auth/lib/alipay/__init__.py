...
class BaseAliPay(object):
    ...
    def build_body(
        ...
        if "grant_type" in biz_content:
            data["grant_type"] = "authorization_code"

        if "code" in biz_content:
            data["code"] = biz_content["code"]

        if "auth_token" in biz_content:
            data["auth_token"] = biz_content["auth_token"]

        # -改动截止

        if append_auth_token:
            # 改动
            data["auth_token"] = self.app_auth_token
...
class ISVAliPay(BaseAliPay):
    ...
    def app_auth_token(self):
        ...
        if not self._app_auth_token:
            result = self.api_alipay_open_auth_token(self._app_auth_code)
    ...
    def api_alipay_open_auth_token(self, refresh_token=None):
        """
        response = {
          "code": "10000",
          "msg": "Success",
        ...
        raw_string = urlopen(url, timeout=15).read().decode("utf-8")
        # -改动
        _res = json.loads(raw_string)
        _res = _res["alipay_system_oauth_token_response"]
        return _res
        # -改动截止
        return self._verify_and_return_sync_response(
    ...
    def api_alipay_open_auth_token_query(self):
        biz_content = {
            #改动
        ...
        raw_string = urlopen(url, timeout=15).read().decode("utf-8")
        #改动
        return json.loads(raw_string)["alipay_user_info_share_response"]
        return self._verify_and_return_sync_response(
