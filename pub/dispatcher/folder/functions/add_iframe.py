...
def add_resource_iframe(r,f,p):

    if not verify.can_create(r,s.RESOURCE_TYPE_IFRAME):
    ...
    try:
        ...
        form = kv.form_resource_iframe(r.POST)
        ...
        title = '云实验室 - 网页Wrapper'

        if not verify.is_valid_key(key,s.RESOURCE_TYPE_IFRAME):
        ...
        # record
        resource.resource_type.objects.create(key=key,type=s.RESOURCE_TYPE_IFRAME)
        resource.resource_iframe.objects.create(key=key,value=value,title=title)
        ...
        verify.did_create(r,s.RESOURCE_TYPE_IFRAME)
        ...
        return j.dic({'error':'' if s.RELEASE else e})
